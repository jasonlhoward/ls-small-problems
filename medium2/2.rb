...
def find_and_delete?(char, blocks)
  p char
  blocks.each_with_index do |block, index|
    if block.split(':').include?(char)
...
def block_word?(word)
  blocks = ['B:O', 'X:K', 'D:Q', 'C:P', 'N:A', 'G:T', 'R:E', 'F:S', 'J:W',
            'H:U', 'V:I', 'L:Y', 'Z:M']
  word.upcase.chars.each do |letter|
    return false if find_and_delete?(letter, blocks) == false
  end
  true
end
...
# block. We also want to make sure that no block is used more than once. If
# both those conditions are met, then we have a valid block word.
