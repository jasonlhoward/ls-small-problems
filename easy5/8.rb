# Alphabetical Numbers

# Write a method that takes an Array of Integers between 0 and 19,
# and returns an Array of those Integers sorted based on the English words
# for each number:

# zero, one, two, three, four, five, six, seven, eight, nine, ten, eleven,
# twelve, thirteen, fourteen, fifteen, sixteen, seventeen, eighteen, nineteen

# Examples:

# alphabetic_number_sort((0..19).to_a) == [
#   8, 18, 11, 15, 5, 4, 14, 9, 19, 1, 7, 17,
#   6, 16, 10, 13, 3, 12, 2, 0
# ]

ENGLISH_NUMBERS = %w[zero one two three four five six seven eight nine ten
                     eleven twelve thirteen fourteen fifteen sixteen
                     seventeen eighteen nineteen].freeze

def my_alphabetic_number_sort(numbers)
  sorted_english_numbers = ENGLISH_NUMBERS.sort
  numbers
    .map! { |num| sorted_english_numbers[num] }
    .map! { |word| ENGLISH_NUMBERS.index(word) }
end

def solution_alphabetic_number_sort(numbers)
  numbers.sort_by { |number| ENGLISH_NUMBERS[number] }
end

p my_alphabetic_number_sort((0..19).to_a) == [
  8, 18, 11, 15, 5, 4, 14, 9, 19, 1, 7, 17,
  6, 16, 10, 13, 3, 12, 2, 0
]

p solution_alphabetic_number_sort((0..19).to_a) == [
  8, 18, 11, 15, 5, 4, 14, 9, 19, 1, 7, 17,
  6, 16, 10, 13, 3, 12, 2, 0
]
