...
# However, what happens when you get into really large numbers? It probably
# took some time for your computer to calculate the last digit of the
# 1,000,007th Fibonacci number. You probably gave up trying to compute the
# last digit of the 123,456,789th Fibonacci number. That's where Solution 2
# comes in.
...
# To compute the last digit of the nth Fibonacci number, you only need the
# last digit of the nth - 1 and nth -2 numbers. As a result, you only ever
# need the last digit of any intermediate result, which eliminates all of the
# computing effort needed to compute the massive numbers involved. Our second
# solution does exactly this: it only computes and uses the last digit in each
...
# the number? You should be able to return results almost instantly.
